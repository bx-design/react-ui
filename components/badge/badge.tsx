...
import { BaseChildrenProps } from 'components/types'

interface BadgeProps extends BaseChildrenProps {
  variant: 'primary' | 'secondary' | 'success' | 'danger' | 'warning' | 'info' | 'dark'
  rounded?: boolean
}

const BadgeComponentRef = forwardRef<HTMLSpanElement, BadgeProps>(function BadgeComponent(
  { children, variant = 'primary', rounded = false }: BadgeProps,
  ref,
) {
  return (
    <span
